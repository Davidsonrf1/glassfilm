        ...
        public void excluir(string codigo)
        {
            if (DBManager.conectado())
            {
                try
                {
                    ...
                    DBManager.EliminaRegistro("MODELO", codigo);
                }
                catch (Exception ex)
                {
                    Logs.Log(ex.Message);
        ...
        public void gravar(string tipo = "")
        {
            if (tipo.Length == 0 || tipo == "Novo")
            {
                if (DBManager.conectado())
                {
                    try
                    {
                        string _sql = " INSERT INTO modelo (CODIGO_MODELO,CODIGO_MARCA,MODELO, SINCRONIZAR) " +
                                      " VALUES (@codigo_modelo,@codigo_marca,@modelo, 1) ";
